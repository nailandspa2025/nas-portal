...
const PackageSelect: React.FC<Props> = ({
  ...
  const { data = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res: any = await PackageApi.getWithPagination(
  ...
  const { data: extraData } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (valuesArray.length === 1) {
        const res: any = await PackageApi.getById(valuesArray[0]);
        return res?.data ? [res.data] : [];
      } else {
        const res: any = await PackageApi.getByIds(valuesArray.join(","));
        return res?.data || [];
      }
    },

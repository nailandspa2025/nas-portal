...
export const CategoryApi = {
  getWithPagination: async (queryString: any) => {
    const endpoint = `/catalog/api/v1/categories/pagingation?${queryString}`;
    return await apiCall(API_METHOD.GET, endpoint);
  },
  getById: async (id: number) => {
    const endpoint = `/catalog/api/v1/categories/${id}`;
    return await apiCall(API_METHOD.GET, endpoint);
  },
  getByIds: async (ids: string) => {
    const endpoint = `/catalog/api/v1/categories/ids/${ids}`;
    return await apiCall(API_METHOD.GET, endpoint);
  },
  create: async (payload: any) => {
    const endpoint = `/catalog/api/v1/categories`;

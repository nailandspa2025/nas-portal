...
export const LoyaltyPointApi = {
  getWithPagination: async (queryString: any) => {
    const endpoint = `/loyalty/api/v1/loyaltysettings/pagingation?${queryString}`;
    return await apiCall(API_METHOD.GET, endpoint);
  },
  getById: async (id: number) => {
    const endpoint = `/loyalty/api/v1/loyaltysettings/${id}`;
    return await apiCall(API_METHOD.GET, endpoint);
  },
  create: async (payload: any) => {
    const endpoint = `/loyalty/api/v1/loyaltysettings`;

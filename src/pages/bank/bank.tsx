...
import * as utils from "../../utils/filter/bank.account";
...
const BankAccounts = () => {
  ...
  const { data, refetch } = useQuery({
    queryKey: ["bankList", { pageNumber, pageSize, ...filters }],
    queryFn: async () => {
      const response: any = await BankAccountApi.getWithPagination(
  ...
  const columns = useMemo(() => {
    return utils.columns({
      hasEditPermission: checkAccessRight(accesses, "update", "bank"),
      hasDeletePermission: checkAccessRight(accesses, "delete", "bank"),
      t,

...
import * as utils from "../../utils/filter/packages";
...
const Packages = () => {
  ...
  const handleActions = {
    createNew: () => {
      navigate("/package/none");
    },
  };
  const handleItemTable = {
    handleEdit: (record: any) => {
      navigate(`/package/${record.id}`);
    },
  ...
  const { data, refetch } = useQuery({
    queryKey: ["packageList", { pageNumber, pageSize, ...filters }],
    queryFn: async () => {
      const response: any = await PackageApi.getWithPagination(
  ...
  const columns = useMemo(() => {
    return utils.columns({
      hasEditPermission: checkAccessRight(accesses, "update", "package"),
      hasDeletePermission: checkAccessRight(accesses, "delete", "package"),
      t,

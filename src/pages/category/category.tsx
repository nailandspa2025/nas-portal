...
import * as utils from "../../utils/filter/categories";
...
const Categories = () => {
  ...
  const { data, refetch } = useQuery({
    queryKey: ["categoryList", { pageNumber, pageSize, ...filters }],
    queryFn: async () => {
      const response: any = await CategoryApi.getWithPagination(
  ...
  const columns = useMemo(() => {
    return utils.columns({
      hasEditPermission: checkAccessRight(accesses, "update", "category"),
      hasDeletePermission: checkAccessRight(accesses, "delete", "category"),
      t,

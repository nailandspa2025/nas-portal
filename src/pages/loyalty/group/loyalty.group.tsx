...
import * as utils from "../../../utils/filter/loyalty.groups";
...
const LoyaltyGroups = () => {
  ...
  const handleActions = {
    createNew: () => {
      navigate("/loyalty-group/none");
    },
  };
  const handleItemTable = {
    handleEdit: (record: any) => {
      navigate(`/loyalty-group/${record.id}`);
    },
  ...
  const columns = useMemo(() => {
    return utils.columns({
      hasEditPermission: checkAccessRight(accesses, "update", "loyalty-group"),
